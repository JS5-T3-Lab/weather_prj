...
const ACTIVITY_FIELD_MAP = {
  ...
  Fishing:    ["tempMaxMin","feelsLikeAvg","windMax","windGustMax","windDegAvg",
               "pressureAvg","humidityAvg","cloudsAvg","popMax","precipSum","visAvg"],

  // 등산/트레킹: 체감온도·습도·풍속·돌풍·가시거리·강수(비+눈)
  Hiking:     ["tempMaxMin","feelsLikeAvg","humidityAvg","windMax","windGustMax",
               "visAvg","popMax","precipSum","snowSum"],

  // 조깅/러닝: 체감온도·습도·풍속·강수확률
  Running:    ["tempMaxMin","feelsLikeAvg","humidityAvg","windMax","popMax","precipSum"],

  // 자전거 라이딩: 풍속·돌풍·풍향·가시거리·강수·기온·체감
  Cycling:    ["tempMaxMin","feelsLikeAvg","windMax","windGustMax","windDegAvg",
               "visAvg","popMax","precipSum"],

  // 야외수영/해수욕: 기온·체감·풍속·강수확률·구름
  Swimming:   ["tempMaxMin","feelsLikeAvg","windMax","popMax","cloudsAvg"],

  // 캠핑/피크닉: 기온(최고/최저)·강수·풍속·습도·구름
  Camping:    ["tempMaxMin","feelsLikeAvg","humidityAvg","windMax","cloudsAvg",
               "popMax","precipSum"],

  // 골프: 풍속·돌풍·강수확률·기온·체감·가시거리
  Golf:       ["tempMaxMin","feelsLikeAvg","windMax","windGustMax","popMax",
               "precipSum","visAvg"],

  // 야외 구기 (축구·배구 등): 강수확률·풍속·기온·체감
  OutdoorSports: ["tempMaxMin","feelsLikeAvg","windMax","popMax","precipSum"],
...
function groupForecastByDate(forecastData){
  ...
  Object.keys(dateMap).forEach(key => {
    ...
    const pressure = items.map(x => x?.main?.pressure).filter(v => v != null);
    ...
    dateMap[key].summary = {
      ...
      pressureAvg:  pressure.length ? pressure.reduce((a,b)=>a+b,0)/pressure.length : null,
      ...
      windDegAvg:   windDeg.length  ? windDeg.reduce((a,b)=>a+b,0)/windDeg.length : null,
...
function getCellValue(varKey, it){
  switch(varKey){
    ...
    case "windDeg":  return it?.wind?.deg != null ? `${it.wind.deg}°` : "-";
    case "humidity": return fmtNumber(it?.main?.humidity, "%");
    case "pressure": return fmtNumber(it?.main?.pressure, " hPa");
    case "clouds":   return fmtNumber(it?.clouds?.all, "%");
    case "vis":      return it?.visibility != null ? `${Math.round(it.visibility/100)/10} km` : "-";
    default:         return "-";
  }
}
...
function getSummaryValue(fieldKey, summary){
  ...
  switch(fieldKey){
    ...
    case "snowSum":      return summary.snowSum > 0 ? fmtNumber(summary.snowSum, " mm") : "-";
    case "windMax":      return fmtNumber(summary.windMax, " m/s");
    case "windGustMax":  return fmtNumber(summary.windGustMax, " m/s");
    case "windDegAvg":   return summary.windDegAvg != null ? `${Math.round(summary.windDegAvg)}°` : "-";
    case "humidityAvg":  return fmtNumber(summary.humidityAvg, "%");
    case "pressureAvg":  return fmtNumber(summary.pressureAvg, " hPa");
    case "cloudsAvg":    return fmtNumber(summary.cloudsAvg, "%");
    case "visAvg":       return summary.visAvg != null ? `${Math.round(summary.visAvg / 100) / 10} km` : "-";
    default:             return "-";
  }
}
  ...
  weeks.forEach((wk, index) => {
    ...
    card.querySelectorAll("th[data-datekey]").forEach(th => {
      th.addEventListener("click", () => {
        ...
        if(!hasData){
          showMessage("error", "이 날짜는 OpenWeather 예보 제공 범위를 벗어나 데이터가 없습니다.");
          return;
        }
...
$form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if(hasAny === 0){
      ...
      showMessage("ok", "예보 데이터를 불러왔습니다. 날짜를 클릭하면 3시간 상세 예보를 볼 수 있습니다.");
    }
